...
# 
(($) ->
  $.fn.emoticonize = (options) ->
    opts = $.extend({}, $.fn.emoticonize.defaults, options)
    escapeCharacters = [")", "(", "*", "[", "]", "{", "}", "|", "^", "<", ">", "\\", "?", "+", "=", "."]
    ...
    preMatch = "(^|[\\s\\0])"
    i = threeCharacterEmoticons.length - 1
    ...
    while i >= 0
      ...
      --i
    i = twoCharacterEmoticons.length - 1
    ...
    while i >= 0
      ...
      --i
    for emoticon of specialEmoticons
    ...
    exclude += "," + opts.exclude  if opts.exclude
    excludeArray = exclude.split(",")
    @not(exclude).each ->
      container = $(this)
      cssClass = "css-emoticon"
      cssClass += " un-transformed-emoticon animated-emoticon"  if opts.animate
      for emoticon of specialEmoticons
      ...
      $.each excludeArray, (index, item) ->
        container.find($.trim(item) + " span.css-emoticon").each ->
  ...
  $.fn.unemoticonize = (options) ->
    opts = $.extend({}, $.fn.emoticonize.defaults, options)
    @each ->
      container = $(this)
      container.find("span.css-emoticon").each ->
